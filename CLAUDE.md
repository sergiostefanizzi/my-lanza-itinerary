...
- **Espansione voci** — le voci `vista`/`attività` con contenuto sono cliccabili (clic sull'intera riga, triangolino ▼) per aprire un pannello con immagine del luogo + breve descrizione (cosa si fa · perché visitarlo). Apertura **a fisarmonica** (una alla volta), stato effimero. Immagini da Wikimedia Commons. Applicato finora **solo a G3**. Vedi sezione "Espansione voci".
...
Le voci di tipo **`vista`** e **`attività`** che hanno contenuto (`image` e/o `desc`) sono **espandibili**: clic sull'intera riga → si apre un pannello con **immagine** del luogo + **descrizione breve** (cosa si fa · perché visitarlo). Un **triangolino ▼** a destra della riga (`.item-chevron`, ruota di 180° all'apertura, come il chevron dei giorni) segnala l'espandibilità. Le voci `cibo`/`logistica` (e qualsiasi voce senza `image`/`desc`) non si espandono e non hanno triangolino.
...
**Immagini**: URL remoti da **Wikimedia Commons** (scelta esplicita: niente file locali per ora). Sono thumbnail `upload.wikimedia.org/.../thumb/.../1280px-...` ottenuti via API `imageinfo` di Commons (URL stabili, accenti URL-encoded). Il campo `image` è una semplice stringa: per passare a file locali in futuro basta cambiare il path. **Stato milestone**: applicato **solo a G3** (Jardín de Cactus, Cueva de los Verdes, Jameos del Agua, Mercado de Haría, Piscine di Punta Mujeres). Gli altri giorni vanno fatti uno alla volta, con la stessa procedura (cercare l'immagine su Commons + scrivere `desc`).
