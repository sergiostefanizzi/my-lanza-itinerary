...
**Immagini**: URL remoti da **Wikimedia Commons** (scelta esplicita: niente file locali per ora → servono connessione; offline non si vedono). Sono thumbnail `upload.wikimedia.org/.../thumb/.../1280px-...` ottenuti via API `imageinfo`/`pageimages` di Commons (URL stabili, accenti URL-encoded). Il campo `image` è una semplice stringa: per passare a file locali in futuro basta cambiare il path. **Stato**: applicato a **tutti i giorni** con voci `vista`/`attività` (G1–G7). Unica eccezione: **Playa de la Cera** (G2) ha solo `desc` (nessuna immagine dedicata trovata su Commons): resta comunque espandibile. Immagini "rappresentative" più deboli da rivedere se non piacciono: **Buggy** (Costa Teguise) e **Degustazione** (vigneti de La Geria). Procedura per aggiungerne/sostituirne: cercare il file su Commons via API `imageinfo` e incollare il `thumburl`.
